<form id="form-save-room" class="row g-3" method="POST" action="{{ route('room.store') }}">
    ...
    <div class="col-md-12">
        ...
        <textarea class="form-control" id="lat" name="lat" rows="3" placeholder="ex: window see beach">{{ old('lat') }}</textarea>
        <div id="error_lat" class="text-danger error"></div>
    </div>
    <div class="col-md-12">
        <label for="lon" class="form-label">Lon</label>
        <textarea class="form-control" id="lon" name="lon" rows="3" placeholder="ex: window see beach">{{ old('lon') }}</textarea>
        <div id="error_lon" class="text-danger error"></div>
    </div>
</form>
